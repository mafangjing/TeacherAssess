...
@interface HunLiLingGanViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation HunLiLingGanViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(HunliLingGanDataReturn:) name:@"HunliLingGanDataReturn" object:nil];
...
-(void)HunliLingGanDataReturn:(NSNotification *)noti{
    NSDictionary * dic = noti.object;
    NSLog(@"%@",dic);
    
    self.tuPianDataArray = dic[@"colors"];
...
-(void)getData{
    ...
    [HunliLingGanNetwork GetHunliLingGanDataUrl:HunliLingGanUrl act:@"lgsccolors" cate:[NSString stringWithFormat:@"%d",self.buttonindex] mver:@"3"];
...
-(void)headView{
    ...
    titleLable.text = self.HeadLableString;
...
-(void)setMyConllectionView{
    ...
    //参数2：队列名
    //注册一个collectionViewCell队列
    [self.collectionView registerNib:[UINib nibWithNibName:@"HunliLingGanCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"collectCell"];
...
-(NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    ...
    return ((NSMutableArray *)self.tuPianDataArray[section][@"list"]).count;
}

-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    
    //根据队列名称和队列Cell所在位置，取出相应的Cell
    HunliLingGanCollectionViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"collectCell" forIndexPath:indexPath];
    
    
    [cell setCollectionCellWithDic:self.tuPianDataArray[indexPath.section][@"list"][indexPath.row]];
...
-(void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BPhotoListViewController * PhotolistVc = [[BPhotoListViewController alloc]init];
    
    PhotolistVc.lastDict = self.tuPianDataArray[indexPath.section][@"list"][indexPath.row];
